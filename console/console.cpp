...
int prepare_console(uint64_t oflags, uint64_t iflags) {
...
void write_styled_text(std::ostream& out, const char* text, ConsoleStyle cs) {
    out << "\x1b[" << (int)cs.fg << ';' << (int)cs.bg << 'm';
    if (cs.ts != TextStyle::NoStyle) {
        out << "\x1b[" << (int)cs.ts << 'm';
    }
    out << text;
    out << "\x1b[0m"; // reset terminal attributes
}
